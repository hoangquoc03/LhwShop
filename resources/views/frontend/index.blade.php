...
@section('page-style')
    ...
    <section class="section-margin calc-60px mt-3">
        <div class="container highlight-box">
            ...
            <div class="featured-products-wrapper"
                style="display:flex; overflow-x:auto; gap:1rem; scroll-snap-type:x mandatory; padding-bottom:1rem;">
                @foreach ($featuredProducts as $product)
                    ...
                    <div class="card card-product h-100 border-0 shadow-sm rounded-3 overflow-hidden position-relative"
                        ...
                        <div class="card-product__img position-relative overflow-hidden">
                            <img class="card-img-top" src="{{ asset('storage/uploads/products/' . $product->image) }}"
        ...
        <div class="carousel-layout">
            ...
            <div
              ...
              transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl">
                ...
                <h2 id="product-name" class="text-2xl font-bold text-gray-800 dark:text-white mb-3">
                    {{ $products[0]->product_name ?? '' }}
                </h2>

                <p id="product-desc" class="text-gray-600 dark:text-gray-300 leading-relaxed mb-6">
            ...
            <div class="carousel-container">
                <div class="carousel" id="carousel">
                    ...
                    @foreach ($products as $key => $product)
                        <div class="item" data-name="{{ $product->product_name }}"
                            data-desc="{{ $product->short_description }}" data-link="/product/{{ $product->id }}"
    ...
    <style>
        ...
        .carousel-layout {
            ...
            /* căn giữa ngang */
            gap: 30px;
            /* khoảng cách giữa carousel và info */
            padding: 20px;
            text-align: center;
        }
                ...
                @foreach ($newProducts as $product)
                    ...
                    <div class="col-md-6 col-lg-4 col-xl-3">
                        <div class="card card-product h-100 border-0 shadow-sm rounded-3 overflow-hidden position-relative">
                            ...
                            {{-- Thông tin sản phẩm --}}
                            <div class="card-body text-center">
                                <p class="text-muted small mb-1">{{ $product->category->categories_text ?? 'Danh mục' }}
                                </p>
                                <h5 class="card-title fw-semibold">
                                    <a href="/product/{{ $product->id }}" class="text-dark text-decoration-none">
                                        {{ $product->product_name }}
                        ...
                        @if ($supplier->image)
                            <div class="logo-item">
                                <img src="{{ asset('storage/uploads/suppliers/logo/' . $supplier->image) }}"
                                    alt="{{ $supplier->supplier_text }}" loading="lazy">
                            </div>
                        @endif
                    @endforeach
            ...
            <div class="d-flex flex-wrap justify-content-center custom-gap">
                @foreach ($ProductPost as $item)
                    <div class="post-image-box overflow-hidden rounded-3 shadow-sm">
                        <img src="{{ asset('storage/uploads/posts/' . $item->image) }}" alt="Post Image"
                            class="post-image">
                    </div>
                @endforeach
            </div>
