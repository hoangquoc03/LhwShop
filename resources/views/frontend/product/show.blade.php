    ...
    <div class="row g-4">
        ...
        <div class="col-12">
            <nav aria-label="breadcrumb" class="mb-3">
                <ol class="breadcrumb bg-transparent p-0 mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('frontend.home.index') }}" class="text-decoration-none">Trang chủ</a></li>
                    @if($product->category?->parent?->parent)
                        <li class="breadcrumb-item">
                            <a href="{{ route('products.byCategory', $product->category->parent->parent->id) }}" class="text-decoration-none">
                    ...
                    @if($product->category?->parent)
                        <li class="breadcrumb-item">
                            <a href="{{ route('products.byCategory', $product->category->parent->id) }}" class="text-decoration-none">
                    ...
                    @if($product->category)
                        <li class="breadcrumb-item">
                            <a href="{{ route('products.byCategory', $product->category->id) }}" class="text-decoration-none">
        ...
        <div class="col-lg-6 d-flex flex-column justify-content-between">
            <div>
                ...
                <p class="text-secondary mb-3">{{ $product->category->categories_text ?? 'Danh mục' }}</p>
                ...
                @if($product->variants->count() > 0)
                    <div class="mb-3">
                        <label for="variantSelect" class="form-label fw-semibold">Chọn phiên bản:</label>
                        <select class="form-select" id="variantSelect">
                            @foreach($product->variants as $variant)
                                @php
                                    $variantDiscount = $variant->discount ?? 0;
                                    $variantPriceDiscounted = $variantDiscount > 0 ? $variant->price*(1-$variantDiscount/100) : $variant->price;
                                @endphp
                                <option value="{{ $variant->id }}" data-price="{{ $variantPriceDiscounted }}">
                                    {{ $variant->variant_name }} - {{ number_format($variantPriceDiscounted,0,',','.') }} ₫
                                    @if($variantDiscount>0) ({{ $variantDiscount }}% OFF) @endif
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                @php
                    ...
                    $avgRating = $reviews->count() ? $reviews->avg('rating') : 0;
            ...
            <a href="#" class="btn btn-gradient-primary btn-lg w-100 mt-3 add-to-cart" data-id="{{ $product->id }}">
        ...
        <div class="col-12 mt-5">
            ...
            @forelse($product->reviews as $review)
                <div class="border-bottom py-3 d-flex flex-column gap-1">
...
if(variantSelect){
    variantSelect.addEventListener('change', function(){
        ...
        document.querySelector('.text-danger').textContent = parseInt(price).toLocaleString('vi-VN') + ' ₫';
    });
}
</script>
