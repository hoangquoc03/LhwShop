    ...
    <div class="row g-4" style="margin-top: 20px">
                ...
                <ol class="breadcrumb bg-transparent p-0 mb-0">
                    ...
                    @if($product->category?->parent?->parent)
                        <li class="breadcrumb-item">
                            <a href="{{ route('products.byCategory', $product->category->parent->parent->id) }}" class="text-decoration-none text-muted">
                    ...
                    @if($product->category?->parent)
                        <li class="breadcrumb-item">
                            <a href="{{ route('products.byCategory', $product->category->parent->id) }}" class="text-decoration-none text-muted">
                    ...
                    @if($product->category)
                        <li class="breadcrumb-item">
                            <a href="{{ route('products.byCategory', $product->category->id) }}" class="text-decoration-none text-muted">
                                {{ $product->category->categories_text }}
                            </a>
                        </li>
                    @endif
                    <li class="breadcrumb-item active text-truncate fw-semibold text-dark" aria-current="page" style="max-width:300px;">
                        {{ $product->product_name }}
                    </li>
                </ol>
            </nav>
            ...
            <div class="product-gallery">
                <div class="main-image-display position-relative mb-3 rounded-4 overflow-hidden shadow-sm">
                    ...
                    @if ($hasDiscount && $percentOff > 0)
                        <span class="badge bg-danger position-absolute top-0 end-0 translate-middle-x rounded-pill px-3 py-2 fw-bold me-2 mt-2" style="z-index: 10;style="background: linear-gradient(135deg,#007bff,#00c6ff);">
                            -{{ $percentOff }}%
                        </span>
                    @endif
                </div>
                ...
                <div class="d-flex gap-2 flex-wrap justify-content-center justify-content-lg-start product-thumbnails">
                    <img src="{{ asset('storage/uploads/products/' . $product->image) }}"
                         class="img-thumbnail thumb-image active"
                         data-src="{{ asset('storage/uploads/products/' . $product->image) }}"
                         alt="{{ $product->product_name }}"
                         loading="lazy">
                    @foreach($product->images as $img)
                        @if($img->image)
                            <img src="{{ asset('storage/uploads/products/img/'.$img->image) }}"
                                 class="img-thumbnail thumb-image"
                                 data-src="{{ asset('storage/uploads/products/img/'.$img->image) }}"
        ...
        <div class="col-lg-6 d-flex flex-column">
            <div class="product-info-wrapper">
                <h1 class="fw-bold mb-2 text-dark">{{ $product->product_name }}</h1>
                ...
                <div class="mb-4 d-flex align-items-center gap-1">
                    ...
                    <small class="text-muted ms-1">
                        @if ($reviewsCount > 0)
                            ({{ number_format($avgRating,1) }}/5) - <a href="#reviews-tab" class="text-decoration-none text-muted reviews-link">{{ $reviewsCount }} đánh giá</a>
                        @else
                            (Chưa có đánh giá)
                        @endif
                    </small>
    ...
    border-width: 2px !important;
                ...
                <div class="mb-4">
                    ...
                    <div class="input-group quantity-control" style="width: 150px;">
                        ...
                        <input type="text" class="form-control text-center border-start-0 border-end-0" value="1" id="quantityInput"
                              aria-label="Số lượng" readonly>
                        <button class="btn btn-outline-secondary border-start-0" type="button" id="plusBtn">+</button>
                    </div>
                </div>
            </div>

            {{-- Add to Cart Button --}}
            <button type="button" class="btn btn-primary btn-lg w-100 mt-auto py-3 add-to-cart-btn" data-id="{{ $product->id }}">
        ...
        <div class="col-12 mt-5">
            <ul class="nav nav-tabs nav-fill flex-column flex-sm-row mb-4" id="productTabs" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active fw-bold text-uppercase" id="description-tab" data-bs-toggle="tab" data-bs-target="#description" type="button" role="tab" aria-controls="description" aria-selected="true">Mô tả chi tiết</button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link fw-bold text-uppercase" id="reviews-tab" data-bs-toggle="tab" data-bs-target="#reviews" type="button" role="tab" aria-controls="reviews" aria-selected="false">Đánh giá ({{ $reviewsCount }})</button>
                </li>
            </ul>
                    ...
                    <div class="reviews-list">
                            ...
                            <div class="card mb-3 shadow-sm border-0 review-card">
                                <div class="card-body">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <h6 class="card-title mb-0 fw-bold text-dark">{{ $review->user->name ?? 'Khách' }}</h6>
                        ...
                        @empty
                            <div class="alert alert-info text-center py-4" role="alert">
                                <i class="fas fa-info-circle me-2"></i> Chưa có đánh giá nào cho sản phẩm này. Hãy là người đầu tiên!
                            </div>
                        @endforelse
                    </div>
                    ...
                    <div class="mt-5 p-4 border rounded-3 bg-light">
                        ...
                        <form action="" method="POST" id="reviewForm"> {{-- Cần cập nhật action --}}
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" name="comment" id="comment" rows="5" placeholder="Chia sẻ cảm nhận của bạn về sản phẩm..." required></textarea>
...
<style>
    ...
    .container {
        max-width: 1300px; /* Wider container for a more spacious feel */
    }
    ...
    /* Product Gallery */
    .main-image-display {
        height: 500px; /* Consistent height for main image */
        display: flex;
        align-items: center;
        justify-content: center;
        background-color: #f0f2f5; /* Light background for images with transparency */
    ...
    .thumb-image {
        ...
        object-fit: cover;
        cursor: pointer;
        border: 2px solid #e9ecef; /* Lighter default border */
        border-radius: 8px; /* Slightly more rounded thumbnails */
    ...
    .thumb-image:hover {
        ...
        transform: scale(1.03); /* Slight zoom on hover/active */
    }
    ...
    .product-price {
        font-size: 2.5rem !important; /* Even larger price */
        letter-spacing: -0.5px; /* Tighter letter spacing */
    }

    .discounted-price {
        font-size: 1.1rem; /* Slightly larger strikethrough price */
    }
    ...
    .quantity-control .form-control {
        border-radius: 0; /* Remove border radius for input in group */
    }
    ...
    .nav-tabs .nav-link {
        border: none;
        border-bottom: 3px solid transparent; /* Highlight active tab with thicker border */
    ...
    .nav-tabs .nav-link:hover {
        ...
        border-color: #ced4da; /* Subtle hover border */
    }
    .nav-tabs .nav-link.active {
        color: var(--bs-primary);
        border-color: var(--bs-primary); /* Active tab highlight */
    ...
    @media (max-width: 575.98px) {
        ...
        .nav-tabs.flex-column {
            flex-direction: row !important; /* Keep tabs horizontal on smaller screens too */
        }
        .nav-tabs .nav-item {
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const productTabs = new bootstrap.Tab(document.getElementById('description-tab')); // Initialize Bootstrap Tab
        ...
        if (variantSelect) {
            variantSelect.addEventListener('change', function() {
                ...
                if (discountedPriceSpan) {
                    ...
                    if (price < originalListPrice) { // Compare variant's discounted price with product's list price
        ...
        addToCartBtn.addEventListener('click', function(e) {
            ...
            console.log(`Adding Product ID: ${productId}, Variant ID: ${selectedVariantId}, Quantity: ${quantity} to cart.`);
        ...
        if (ratingStarsContainer) {
            ...
            stars.forEach(star => {
                ...
                star.addEventListener('click', function() {
                    ...
                    ratingStarsContainer.classList.add('rated'); // Add a class to indicate it's rated
                });
            });
