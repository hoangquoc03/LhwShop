    ...
    <div class="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700" bis_skin_checked="1">
        <div class="w-full mb-1" bis_skin_checked="1">
            <div class="mb-4" bis_skin_checked="1">
                ...
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Tất cả kho hàng</h1>
            </div>
            <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700" bis_skin_checked="1">
                ...
                <form method="GET" action="{{ route('backend.ShopStore.index') }}" class="sm:pr-3">
                    ...
                    <div class="flex items-center w-full sm:justify-end" bis_skin_checked="1">
                        <div class="flex pl-2 space-x-1" bis_skin_checked="1">
                            ...
                            <a href="#" id= "btnBatchDelete" data-batch-delete-url="{{ route('backend.ShopStore.batchDelete') }}"
                ...
                <div class="overflow-hidden shadow" bis_skin_checked="1">
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        <tbody id="product-table-body" class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                            @foreach ($dsShopStores as $index => $item)
                                <tr data-search="{{ Str::lower($item->store_name) }}" class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                    <td class="w-4 p-4">
                                        <div class="flex items-center">
                                    ...
                                    <td class="p-4">
                                        <img src="/storage/uploads/stores/logo/{{ $item->image }}" alt="Product Image" class="w-16 h-16 object-cover rounded">
                                    </td>
                                    ...
                                    <td class="p-4 space-x-2 whitespace-nowrap">
                                        ...
                                        data-id="{{ $item->id }}"
                                        data-delete-url = "{{ route('backend.ShopStore.destroy', ['id' => $item->id]) }}"
                                        >
                        ...
                        <div id="drawer-update-product-{{ $item->id }}" class="fixed top-0 right-0 z-40 w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform bg-white dark:bg-gray-800 translate-x-full" tabindex="-1" aria-labelledby="drawer-label-{{ $item->id }}" bis_skin_checked="1" aria-hidden="true">
                            ...
                            <form action="{{ route('backend.ShopStore.update', ['id' => $item->id]) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="space-y-4">
                                    <div bis_skin_checked="1">
                                        <label for="store_code" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Mã kho hàng</label>
                                        <input value="{{ $item->store_code }}" type="text" name="store_code" id="store_code" 
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" 
                                        placeholder="Mã kho hàng" required="">
                                    </div>
                                    <div bis_skin_checked="1">
                                        <label for="store_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tên kho hàng</label>
                                        <input value="{{ $item->store_name }}" type="text" name="store_name" id="store_name" 
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" 
                                    ...
                                    <div>
                                        ...
                                        <img src="{{ asset('storage/uploads/stores/logo/' . $item->image) }}"
                                            data-default="{{ asset('storage/uploads/stores/logo/' . $item->image) }}"
                    ...
                    @php
                        $currentPage = $dsShopStores->currentPage();
                        $prevUrl = $dsShopStores->previousPageUrl()
                            ? request()->fullUrlWithQuery(['page' => $currentPage - 1])
                            : null;

                        $nextUrl = $dsShopStores->nextPageUrl()
                            ? request()->fullUrlWithQuery(['page' => $currentPage + 1])
                            : null;
                    @endphp
                    ...
                    <div class="sticky bottom-0 right-0 w-full p-4 bg-white border-t border-gray-200 sm:flex sm:items-center sm:justify-between dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <div class="flex items-center space-x-3 mb-4 sm:mb-0">
                            ...
                            class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg 
                                    ...
                                    {{ $dsShopStores->onFirstPage() ? 'opacity-50 pointer-events-none' : '' }}">
                                <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg 
                                    hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 
                                    dark:bg-blue-500 dark:hover:bg-blue-600 dark:focus:ring-blue-800 
                                    {{ !$dsShopStores->hasMorePages() ? 'opacity-50 pointer-events-none' : '' }}">
                                Tiếp
                        ...
                        <div class="flex items-center">
                            <span class="text-sm text-gray-500 dark:text-gray-400">
                                ...
                                <span class="font-semibold text-gray-900 dark:text-white">
                                    {{ $dsShopStores->firstItem() ?? 0 }} - {{ $dsShopStores->lastItem() ?? 0 }}
                                </span> 
                                trên tổng 
                                <span class="font-semibold text-gray-900 dark:text-white">
                                    {{ $dsShopStores->total() }}
...
<div id="drawer-create-product-default"
    ...
    <form id="frmCreate" name="frmCreate"
        action="{{ route('backend.ShopStore.store') }}"
        ...
        <div class="space-y-4" bis_skin_checked="1">
                <div class="space-y-4" bis_skin_checked="1">
                    <div bis_skin_checked="1">
                        <label for="store_code" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                            Mã kho hàng</label>
                        <input value="{{ old('store_code') }}" type="text" name="store_code" id="store_code" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" 
                        placeholder="store_code" required="">
                    </div>

                    <div bis_skin_checked="1">
                        <label for="store_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">tên kho hàng</label>
                        <input value="{{ old('store_name') }}" type="text" name="store_name" id="store_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="store_name" required="">
                    </div>

<div class="mb-4">
    <label for="create-image" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
        Hình ảnh
    </label>
    ...
    <input 
        ...
        class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 
               dark:text-gray-200 dark:border-gray-600 dark:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
    >
...
<script>
  ...
  $(function () {
    $('#frmCreate').validate({
        ...
        messages: {
            store_code: {
                required: 'Vui lòng nhập mã kho hàng.',
                minlength: 'Mã kho hàng phải từ 3 ký tự trở lên.',
                maxlength: 'Mã kho hàng phải dưới 100 ký tự.'
            },
            store_name: {
                required: 'Vui lòng nhập tên kho hàng.',
                minlength: 'Tên kho hàng phải từ 3 ký tự trở lên.',
                maxlength: 'Tên kho hàng phải dưới 100 ký tự.'
            },
            description: {
...
<script>
    ...
    const myDropzone = new Dropzone("#frmCreate", {
        url: "{{ route('backend.ShopStore.store') }}", // ép dùng route POST
        method: "post", 
        ...
        init: function () {
            this.on("successmultiple", function (files, response) {
                Toastify({
                    ...
                    close: true,
                    onClick: function () {
                        window.location.href = "{{ route('backend.ShopStore.index') }}";
                    }
                }).showToast();
