    ...
    <div class="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700" bis_skin_checked="1">
        <div class="w-full mb-1" bis_skin_checked="1">
                    ...
                    <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                        ...
                        <li>
                            <div class="flex items-center" bis_skin_checked="1">
                                ...
                                <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500" aria-current="page">Product Discount</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Tất cả giảm giá sản phẩm</h1>
            </div>
            <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700" bis_skin_checked="1">
                ...
                <form method="GET" action="{{ route('backend.ProductDiscount.index') }}" class="sm:pr-3">
                    ...
                    <div class="flex items-center w-full sm:justify-end" bis_skin_checked="1">
                        <div class="flex pl-2 space-x-1" bis_skin_checked="1">
                            ...
                            <a href="#" id= "btnBatchDelete" data-batch-delete-url="{{ route('backend.ProductDiscount.batchDelete') }}"
                ...
                <div class="overflow-hidden shadow" bis_skin_checked="1">
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        <tbody id="product-table-body" class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                            @foreach ($dsShopProductDiscounts as $index => $item)
                                <tr data-search="{{ Str::lower($item->product->product_name) }}" class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                        ...
                                        @if($item->is_fixed)
                                            <span class="inline-flex items-center gap-1.5 px-3 py-1 text-sm font-semibold text-green-800 bg-green-100 border border-green-300 rounded-full shadow-sm dark:bg-green-900 dark:text-green-300 dark:border-green-700">
                                                ...
                                                {{-- <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8V4m0 16v-4m-6 0h12" />
                                                </svg> --}}
                                    ...
                                    <td class="p-4 space-x-2 whitespace-nowrap">
                                        ...
                                        <button type="button"
                                            ...
                                            data-discount_name ="{{ $item->discount_name  }}"
                                        ...
                                        data-id="{{ $item->id }}"
                                        data-delete-url = "{{ route('backend.ProductDiscount.destroy', ['id' => $item->id]) }}"
                                        >
                        ...
                        @foreach ($dsShopProductDiscounts as $item)
                        <div id="drawer-update-product-{{ $item->id }}" class="fixed top-0 right-0 z-40 w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform bg-white dark:bg-gray-800 translate-x-full" tabindex="-1" aria-labelledby="drawer-label-{{ $item->id }}" bis_skin_checked="1" aria-hidden="true">
                            ...
                            <form action="{{ route('backend.ProductDiscount.update', ['id' => $item->id]) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="space-y-4">
                                    ...
                                    {{-- Chọn sản phẩm --}}
                                    <div>
                                        <label for="product_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Sản phẩm áp dụng</label>
                                    ...
                                    {{-- Tên giảm giá --}}
                                    <div>
                                        <label for="discount_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tên giảm giá</label>
                                        <input value="{{ $item->discount_name }}" type="text" name="discount_name" id="discount_name"
                    ...
                    @php
                        $currentPage = $dsShopProductDiscounts->currentPage();
                        $prevUrl = $dsShopProductDiscounts->previousPageUrl()
                            ? request()->fullUrlWithQuery(['page' => $currentPage - 1])
                            : null;

                        $nextUrl = $dsShopProductDiscounts->nextPageUrl()
                            ? request()->fullUrlWithQuery(['page' => $currentPage + 1])
                            : null;
                    @endphp
                    ...
                    <div class="sticky bottom-0 right-0 w-full p-4 bg-white border-t border-gray-200 sm:flex sm:items-center sm:justify-between dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <div class="flex items-center space-x-3 mb-4 sm:mb-0">
                            ...
                            class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg 
                                    ...
                                    {{ $dsShopProductDiscounts->onFirstPage() ? 'opacity-50 pointer-events-none' : '' }}">
                                <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg 
                                    hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 
                                    dark:bg-blue-500 dark:hover:bg-blue-600 dark:focus:ring-blue-800 
                                    {{ !$dsShopProductDiscounts->hasMorePages() ? 'opacity-50 pointer-events-none' : '' }}">
                                Tiếp
                        ...
                        <div class="flex items-center">
                            <span class="text-sm text-gray-500 dark:text-gray-400">
                                ...
                                <span class="font-semibold text-gray-900 dark:text-white">
                                    {{ $dsShopProductDiscounts->firstItem() ?? 0 }} - {{ $dsShopProductDiscounts->lastItem() ?? 0 }}
                                </span> 
                                trên tổng 
                                <span class="font-semibold text-gray-900 dark:text-white">
                                    {{ $dsShopProductDiscounts->total() }}
                                </span> mục
                            </span>
                        </div>
...
<div id="drawer-create-product-default"
    ...
    <form id="frmCreateDiscount" name="frmCreateDiscount"
        action="{{ route('backend.ProductDiscount.store') }}"
        ...
        <!-- Tên giảm giá -->
        <div>
            <label for="discount_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                Tên giảm giá
            </label>
