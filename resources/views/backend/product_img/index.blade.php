...
@section('main-content')
    <div class="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700"
        ...
        <div class="w-full mb-1" bis_skin_checked="1">
            <div class="mb-4" bis_skin_checked="1">
                ...
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Tất cả hình ảnh sản phẩm</h1>
            </div>
            <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700"
                bis_skin_checked="1">
                <div class="flex items-center mb-4 sm:mb-0" bis_skin_checked="1">
                    <form method="GET" action="{{ route('backend.ProductImg.index') }}" class="sm:pr-3">
                    ...
                    <div class="flex items-center w-full sm:justify-end" bis_skin_checked="1">
                        <div class="flex pl-2 space-x-1" bis_skin_checked="1">
                            ...
                            <a href="#" id= "btnBatchDelete"
                                data-batch-delete-url="{{ route('backend.ProductImg.batchDelete') }}"
            ...
            <div class="inline-block min-w-full align-middle" bis_skin_checked="1">
                <div class="overflow-hidden shadow" bis_skin_checked="1">
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        @php
                            $groupedProductImages = $dsProductImg->groupBy('product_id');
                        @endphp

                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                            @foreach ($groupedProductImages as $productId => $images)
                                @php
                                    $product = $images->first()->product;
                                @endphp

                                <tr data-search="{{ Str::lower($product->product_name) }}"
                                    class="hover:bg-gray-100 dark:hover:bg-gray-700">

                                    {{-- CHECKBOX --}}
                                    <td class="w-4 p-4">
                                        <input type="checkbox" name="listSelectedProductIds[]"
                                            value="{{ $productId }}" class="w-4 h-4 border-gray-300 rounded">
                                    </td>
                                   ...
                                   w-5 h-5 text-xs text-white bg-red-600 rounded-full btn-delete"
                                                        data-delete-url="{{ route('backend.ProductImg.destroy', ['id' => $img->id]) }}">
                                                        ×
                                                    </button>
                                                </div>
                                            @endforeach
                                    ...
                                    <td class="p-4 whitespace-nowrap">
                                        <button type="button" id="updateProductButton"
                                            ...
                                            data-post_image="{{ $product->post_image }}"
                                            data-post_slug="{{ $product->post_slug }}"
                                            data-post_title="{{ $product->post_title }}"
                                            data-post_type="{{ $product->post_type }}"
                                            data-post_status="{{ $product->post_status }}">
                                        ...
                                        <button type="button"
                                            ...
                                            data-id="{{ $product->id }}"
                                            data-delete-url="{{ route('backend.post.destroy', ['id' => $product->id]) }}">
                                            <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
                        ...
                        @foreach ($groupedProductImages as $productId => $images)
                            ...
                            <div id="drawer-update-product-{{ $product->id }}"
                                ...
                                tabindex="-1" aria-labelledby="drawer-label-{{ $product->id }}" bis_skin_checked="1"
                                aria-hidden="true">

                                <h5 id="drawer-label-{{ $product->id }}"
                                    class="inline-flex items-center mb-6 text-sm font-semibold text-gray-500 uppercase dark:text-gray-400">
                                    Cập nhật - {{ $product->id }}</h5>

                                <button type="button" data-drawer-dismiss="drawer-update-product-{{ $product->id }}"
                                ...
                                <div class="space-y-4 mt-4">
                                    @foreach ($images as $item)
                                        <div class="flex items-center gap-3">
                                            <img src="/storage/uploads/products/img/{{ $item->image }}"
                                                class="w-20 h-20 object-cover rounded border">

                                            <form action="{{ route('backend.ProductImg.update', $item->id) }}"
                                                method="POST" enctype="multipart/form-data">
                                                @csrf
                                                @method('PUT')

                                                <input type="file" name="image" class="text-sm mb-2">

                                                <div class="bottom-0 left-0 flex justify-center w-full pb-4 mt-4 space-x-4 sm:absolute sm:px-4 sm:mt-0"
                                                    bis_skin_checked="1">
                                                    <button data-url="{{ route('backend.ProductImg.index') }}"
                                                        type="submit"
    ...
    <div id="drawer-create-product-default"
        ...
        <!-- FORM -->
        <form id="frmCreate" name="frmCreate" action="{{ route('backend.ProductImg.store') }}"
            ...
            <!-- Kéo thả ảnh -->
            <div
                class="dz-message flex flex-col items-center justify-center text-sm text-gray-700 dark:text-gray-300 py-12 px-6 border-2 border-dashed border-gray-400 rounded-md bg-gray-200 dark:bg-gray-700 transition-all hover:bg-gray-300 dark:hover:bg-gray-600 cursor-pointer">
                    ...
                    Toastify({
                        ...
                        onClick: function() {
                            window.location.href =
                                "{{ route('backend.ProductImg.index') }}";
                        }
                    }).showToast();
