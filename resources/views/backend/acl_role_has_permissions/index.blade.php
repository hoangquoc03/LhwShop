    ...
    <div class="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700" bis_skin_checked="1">
        <div class="w-full mb-1" bis_skin_checked="1">
            <div class="mb-4" bis_skin_checked="1">
                ...
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Danh sách các quyền theo vai trò</h1>
            </div>
...
<table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
    <thead class="bg-gray-100 dark:bg-gray-700">
        <tr>
            ...
            <th class="p-4 text-sm font-semibold text-left text-gray-600 uppercase dark:text-gray-300">Quyền đã cấp</th>
            <th class="p-4 text-sm font-semibold text-left text-gray-600 uppercase dark:text-gray-300">Hành động</th>
        </tr>
    </thead>
    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
        @foreach ($groupedAclRoleHasPermissions as $roleName => $permissions)
...
@foreach ($groupedAclRoleHasPermissions as $roleName => $permissions)
    ...
    <div id="drawer-update-role-{{ $roleItem->role_id }}"
        ...
        <form data-role-id="{{ $roleItem->role_id }}" action="{{ route('backend.acl_role_has_permissions.store') }}" method="POST">
            @csrf
            <div class="space-y-4">
                <div>
                    <label for="role_name_{{ $roleItem->role_id }}" class="block mb-2 text-sm font-semibold text-gray-700 dark:text-gray-300">Tên vai trò</label>
                    <input value="{{ $roleItem->role->name }}" type="text" name="role_name" id="role_name_{{ $roleItem->role_id }}"
                        class="w-full p-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" required>
                    <input type="hidden" name="role_id" value="{{ $roleItem->role_id }}">
                </div>

                <div>
                    <label class="block mb-2 text-sm font-semibold text-gray-700 dark:text-gray-300">Phân quyền</label>
                    @foreach ($aclPermissions as $i => $item)
                        <label class="flex items-center space-x-2">
                            <input type="checkbox" name="permission_id[]" id="permission_id_{{ $roleItem->role_id }}_{{ $i }}"
                                value="{{ $item->id }}"
            ...
            <div class="bottom-0 left-0 flex justify-center w-full pb-4 mt-4 space-x-4 sm:absolute sm:px-4 sm:mt-0">
                ...
                <button type="button"
                    ...
                    data-drawer-dismiss="drawer-update-role-{{ $roleItem->role_id }}">
                    Hủy
                </button>
            </div>
        </form>
