...
@section('main-content')
                    ...
                    <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                        ...
                        <li>
                            <div class="flex items-center" bis_skin_checked="1">
                                ...
                                <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500" aria-current="page">Supplier</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Tất cả nhà danh mục 2</h1>
            </div>
                ...
                <div class="overflow-hidden shadow" bis_skin_checked="1">
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                            ...
                            @foreach ($dsShopSuppliers as $item)
                                ...
                                <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <img src="/storage/uploads/suppliers/logo/{{ $item->image }}" alt="Post Image" class="w-16 h-16 object-cover rounded">
                                </td>
                                <td class="max-w-sm p-4 overflow-hidden text-base font-normal text-gray-500 truncate xl:max-w-xs dark:text-gray-400">
                                    {{ $item->supplier_code }}
                                </td>
                                <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $item->supplier_text}}</td>
                                <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $item->description }}</td>
                                <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $item->created_at->diffForHumans() }}
                                </td>
                                <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $item->updated_at == NULL ? '': $item->updated_at->diffForHumans() }}
                                </td>
                                <td class="p-4 space-x-2 whitespace-nowrap">
                                    ...
                                    data-supplier_text="{{ $item->supplier_text }}"
                                    ...
                                    <button type="button" id="deleteProductButton" 
                                        ...
                                        data-id="{{ $item->id }}"
                                        data-delete-url = "{{ route('backend.Supplier.destroy', ['id' => $item->id]) }}"
                                        >
                                        <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            <path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                                        </svg>
                        ...
                        <div id="drawer-update-product-{{ $item->id }}" class="fixed top-0 right-0 z-40 w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform bg-white dark:bg-gray-800 translate-x-full" tabindex="-1" aria-labelledby="drawer-label-{{ $item->id }}" bis_skin_checked="1" aria-hidden="true">
                                ...
                                Cập nhật Nhà danh mục 2- {{ $item->id }}</h5>
                            ...
                            <form action="{{ route('backend.Supplier.update',['id' => $item->id]) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="space-y-4" bis_skin_checked="1">
                                    <div bis_skin_checked="1">
                                        <label for="supplier_code" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Mã nhà danh mục 2</label>
                                        <input value="{{ $item->supplier_code }}" type="text" name="supplier_code" id="supplier_code" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="supplier_code" required="">
                                    </div>

                                    <div bis_skin_checked="1">
                                        <label for="supplier_text" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Văn bản Nhà danh mục 2</label>
                                        <input value="{{ $item->supplier_text }}" type="text" name="supplier_text" id="supplier_text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="supplier_text" required="">
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <img 
                                            src="{{ asset('storage/uploads/suppliers/logo/' . $item->image) }}"
                                            data-default="{{ asset('storage/uploads/suppliers/logo/' . $item->image) }}"
                                ...
                                <div class="bottom-0 left-0 flex justify-center w-full pb-4 mt-4 space-x-4 sm:absolute sm:px-4 sm:mt-0" bis_skin_checked="1">
                                    <button data-url="{{ route('backend.Supplier.index') }}" type="submit" class="w-full justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                                        Cập nhật
                                    </button>
                                    <button data-url="{{ route('backend.Supplier.index') }}" type="button" class="w-full justify-center text-red-600 inline-flex items-center hover:text-white border border-red-600 hover:bg-red-600 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900">
    ...
    <div id="drawer-create-product-default" class="fixed top-0 right-0 z-40 w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform bg-white dark:bg-gray-800 translate-x-full" tabindex="-1" aria-labelledby="drawer-label" bis_skin_checked="1" aria-hidden="true">
        <h5 id="drawer-label" class="inline-flex items-center mb-6 text-sm font-semibold text-gray-500 uppercase dark:text-gray-400">Danh mục mới</h5>
        ...
        <form id ="frmCreate" action="{{ route('backend.Supplier.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="space-y-4" bis_skin_checked="1">
                <div class="space-y-4" bis_skin_checked="1">
                    <div bis_skin_checked="1">
                        <label for="supplier_code" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Mã nhà danh mục 2</label>
                        <input value="{{ old('supplier_code') }}" type="text" name="supplier_code" id="supplier_code" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="supplier_code" required="">
                    </div>

                    <div bis_skin_checked="1">
                        <label for="supplier_text" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Văn bản nhà danh mục 2</label>
                        <input value="{{ old('supplier_text') }}" type="text" name="supplier_text" id="supplier_text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="supplier_text" required="">
                    </div>
                    ...
                    <div>
                        ...
                        <img 
                            src="{{ asset('static/dist/images/default-image.jpg') }}"
                    ...
                    <div class="bottom-0 left-0 flex justify-center w-full pb-4 space-x-4 md:px-4 md:absolute" bis_skin_checked="1">
                        <button data-url="{{ route('backend.Supplier.index') }}" type="submit" class="text-white w-full justify-center bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                            Thêm Nhà danh mục 2
                        </button>
                        <button data-url="{{ route('backend.Supplier.index') }}" type="button" data-drawer-dismiss="drawer-create-product-default" aria-controls="drawer-create-product-default" class="inline-flex w-full justify-center text-gray-500 items-center bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-primary-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
...
<script>
  ...
  $(function () {
    $('#frmCreate').validate({
        ...
        messages: {
            supplier_code: {
                required: 'Vui lòng nhập mã nhà danh mục.',
                minlength: 'Mã nhà danh mục phải từ 3 ký tự trở lên.',
                maxlength: 'Mã nhà danh mục phải dưới 100 ký tự.'
            },
            supplier_text: {
                required: 'Vui lòng nhập văn bản nhà danh mục.',
                minlength: 'Văn bản nhà danh mục phải từ 3 ký tự trở lên.',
                maxlength: 'Văn bản nhà danh mục phải dưới 100 ký tự.'
            },
            image: {
                extension: 'Ảnh phải có định dạng jpg, jpeg, png hoặc gif.'
            },
