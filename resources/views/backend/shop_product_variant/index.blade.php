    ...
    <div class="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700" bis_skin_checked="1">
        <div class="w-full mb-1" bis_skin_checked="1">
                    ...
                    <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                        ...
                        <li>
                            <div class="flex items-center" bis_skin_checked="1">
                                ...
                                <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500" aria-current="page">Product Variant</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Tất cả loại sản phẩm</h1>
            </div>
            <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700" bis_skin_checked="1">
                ...
                <form method="GET" action="{{ route('backend.ProductVariant.index') }}" class="sm:pr-3">
                ...
                <div class="overflow-hidden shadow" bis_skin_checked="1">
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        <tbody id="product-table-body" class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                            @foreach ($dsShopProductVariants as $index => $item)
                                <tr data-search="{{ Str::lower($item->product->product_name) }}" class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                    ...
                                    <td class="p-4 space-x-2 whitespace-nowrap">
                                        ...
                                        data-id="{{ $item->id }}"
                                        data-delete-url = "{{ route('backend.ProductVariant.destroy', ['id' => $item->id]) }}"
                                        >
                        ...
                        @foreach ($dsShopProductVariants as $item)
                        <div id="drawer-update-product-{{ $item->id }}" class="fixed top-0 right-0 z-40 w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform bg-white dark:bg-gray-800 translate-x-full" tabindex="-1" aria-labelledby="drawer-label-{{ $item->id }}" bis_skin_checked="1" aria-hidden="true">
                            <h5 id="drawer-label-{{ $item->id }}" class="inline-flex items-center mb-6 text-sm font-semibold text-gray-500 uppercase dark:text-gray-400">
                                Cập nhật loại sản phẩm - {{ $item->id }}</h5>
                            ...
                            <form action="{{ route('backend.ProductVariant.update', ['id' => $item->id]) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="space-y-4">
                                    ...
                                    <div>
                                        <label for="sku" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Mã SKU</label>
                                        <input value="{{ $item->sku }}" type="text" name="sku" id="sku"
                                            ...
                                            placeholder="VD: LHW-DEN-I5-16G" required>
                                    </div>

                                    <div>
                                        <label for="variant_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tên Loại sản phẩm</label>
                                        <input value="{{ $item->variant_name }}" type="text" name="variant_name" id="variant_name"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:text-white dark:border-gray-600"
                                            placeholder="VD: /Den/i5/16G/" required>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        <label for="stock_quantity" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Số lượng</label>
                                        <input value="{{ $item->stock_quantity }}" type="number" name="stock_quantity" id="stock_quantity"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:text-white dark:border-gray-600"
                    ...
                    @php
                        $currentPage = $dsShopProductVariants->currentPage();
                        $prevUrl = $dsShopProductVariants->previousPageUrl()
                            ? request()->fullUrlWithQuery(['page' => $currentPage - 1])
                            : null;

                        $nextUrl = $dsShopProductVariants->nextPageUrl()
                            ? request()->fullUrlWithQuery(['page' => $currentPage + 1])
                            : null;
                    @endphp
                    ...
                    <div class="sticky bottom-0 right-0 w-full p-4 bg-white border-t border-gray-200 sm:flex sm:items-center sm:justify-between dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <div class="flex items-center space-x-3 mb-4 sm:mb-0">
                            ...
                            class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg 
                                    ...
                                    {{ $dsShopProductVariants->onFirstPage() ? 'opacity-50 pointer-events-none' : '' }}">
                                <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg 
                                    hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 
                                    dark:bg-blue-500 dark:hover:bg-blue-600 dark:focus:ring-blue-800 
                                    {{ !$dsShopProductVariants->hasMorePages() ? 'opacity-50 pointer-events-none' : '' }}">
                                Tiếp
                        ...
                        <div class="flex items-center">
                            <span class="text-sm text-gray-500 dark:text-gray-400">
                                ...
                                <span class="font-semibold text-gray-900 dark:text-white">
                                    {{ $dsShopProductVariants->firstItem() ?? 0 }} - {{ $dsShopProductVariants->lastItem() ?? 0 }}
                                </span> 
                                trên tổng 
                                <span class="font-semibold text-gray-900 dark:text-white">
                                    {{ $dsShopProductVariants->total() }}
                                </span> mục
                            </span>
                        </div>
...
<div id="drawer-create-product-default"
    ...
    <form id="frmCreate" name="frmCreate"
        action="{{ route('backend.ProductVariant.store') }}"
        ...
        <div>
            ...
            <input type="text" id="sku" name="sku" value="{{ old('sku') }}"
                class="input-style"
                placeholder="VD: LHW-DEN-I5-16G"
                required>
        </div>
        <!-- variant_name -->   
        <div>
            <label for="variant_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                Tên loại sản phẩm
            </label>
            <input type="text" id="variant_name" name="variant_name" value="{{ old('variant_name') }}"
                class="input-style"
                placeholder="VD: /Den/i5/16G/"
                required>
        </div>
        <!-- price -->
        <div>
        ...
        <!-- stock_quantity -->
        <div>
            <label for="stock_quantity" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                Số lượng
            </label>
...
<script>
    ...
    const myDropzone = new Dropzone("#frmCreate", {
        url: "{{ route('backend.ProductVariant.store') }}", // ép dùng route POST
        method: "post", 
        ...
        init: function () {
            this.on("successmultiple", function (files, response) {
                Toastify({
                    ...
                    close: true,
                    onClick: function () {
                        window.location.href = "{{ route('backend.ProductVariant.index') }}";
                    }
                }).showToast();
